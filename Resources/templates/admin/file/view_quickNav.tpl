{* purpose of this template: files view filter form in admin area *}
{checkpermissionblock component='MUFiles:File:' instance='::' level='ACCESS_EDIT'}
{assign var='objectType' value='file'}
<form action="{$modvars.ZConfig.entrypoint|default:'index.php'}" method="get" id="mufilesFileQuickNavForm" class="mufilesQuickNavForm">
    <fieldset>
        ...
        {if !isset($collectionFilter) || $collectionFilter eq true}
            ...
            {foreach item='option' from=$listEntries}
                ...
                <option value="{$entryId}"{if $entryId eq $aliascollection} selected="selected"{/if}>{$option.name}</option>
            {/foreach}
            </select>
        {/if}
...
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        mufilesInitQuickNavigation('file', 'admin');
