 ...
 *
 * This is the base repository class for hookobject entities.
 */
class MUFiles_Entity_Repository_Base_Hookobject extends EntityRepository
{
    /**
     * @var string The default sorting field/expression.
     */
    protected $defaultSortingField = 'hookedModule';
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('hookobject', 'workflowState');
            }
            ...
            if (in_array($args['action'], array('display', 'view'))) {
                ...
                $templateParameters['relationThumbPreset'] = $imageHelper->getCustomPreset('', '', 'MUFiles_relateditem', $context, $args);
            }
        }
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MUFiles_Entity_Hookobject', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUFiles_Entity_Hookobject', 'tbl')
     ...
     *
     * @return array|MUFiles_Entity_Hookobject retrieved data array or MUFiles_Entity_Hookobject instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved MUFiles_Entity_Hookobject instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                SessionUtil::setVar('MUFilesHookobjectsCurrentPage', $page);
            } else {
                // restore current page from session
                $page = SessionUtil::getVar('MUFilesHookobjectsCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('MUFiles', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'hookobject', 'catids' => $v));
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved hookobjects only
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numHookobjects';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MUFiles_Entity_Hookobject', 'tbl');
     ...
     * @param int    $excludeId  Id of hookobjects to exclude (optional).
     *
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        ...
        $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('MUFiles', 'showOnlyOwnEntries', 0), 'GETPOST');
    ...
    {
        $selection = ', tblCollectionhook, tblFilehook';
    ...
    {
        $qb->leftJoin('tbl.collectionhook', 'tblCollectionhook');
        $qb->leftJoin('tbl.filehook', 'tblFilehook');
