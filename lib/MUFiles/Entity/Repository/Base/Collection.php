 ...
 */
class MUFiles_Entity_Repository_Base_Collection extends EntityRepository
{
    /**
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['inFrontendItems'] = $booleanSelectorItems;
            }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['catIdList'] = ModUtil::apiFunc('MUFiles', 'category', 'retrieveCategoriesFromRequest', array('ot' => 'collection', 'source' => 'GET'));
        $parameters['parent'] = isset($this->controllerArguments['parent']) ? $this->controllerArguments['parent'] : FormUtil::getPassedValue('parent', 0, 'GET');
        ...
        $parameters['inFrontend'] = isset($this->controllerArguments['inFrontend']) ? $this->controllerArguments['inFrontend'] : FormUtil::getPassedValue('inFrontend', '', 'GET');
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUFiles_Entity_Collection', 'tbl')
     ...
     *
     * @return array|MUFiles_Entity_Collection retrieved data array or MUFiles_Entity_Collection instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('MUFiles', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'collection', 'catids' => $v));
            } elseif (in_array($k, array('q', 'searchterm'))) {
                // quick search
                if (!empty($v)) {
                    $qb = $this->addSearchFilter($qb, $v);
                }
            } elseif (in_array($k, array('inFrontend'))) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MUFiles_Entity_Collection', 'tbl');
