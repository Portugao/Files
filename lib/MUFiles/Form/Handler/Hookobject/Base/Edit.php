 ...
 */
class MUFiles_Form_Handler_Hookobject_Base_Edit extends MUFiles_Form_Handler_Common_Edit
{
    /**
     * Pre-initialise hook.
     *
    ...
    public function preInitialize()
    {
        ...
        $this->objectType = 'hookobject';
        $this->objectTypeCapital = 'Hookobject';
        $this->objectTypeLower = 'hookobject';
    ...
    public function initialize(Zikula_Form_View $view)
    {
        ...
        if ($this->mode == 'create') {
            ...
            if (!$modelHelper->canBeCreated($this->objectType)) {
                LogUtil::registerError($this->__('Sorry, but you can not create the hookobject yet as other items are required which must be created before!'));
        ...
        $this->relationPresets['collectionhook'] = FormUtil::getPassedValue('collectionhook', '', 'GET');
        if (!empty($this->relationPresets['collectionhook'])) {
            $relObj = ModUtil::apiFunc($this->name, 'selection', 'getEntity', array('ot' => 'collection', 'id' => $this->relationPresets['collectionhook']));
            if ($relObj != null) {
                $relObj->addHookcollection($entity);
            }
        }
        // editable relation, we store the id and assign it now to show it in UI
        $this->relationPresets['filehook'] = FormUtil::getPassedValue('filehook', '', 'GET');
        if (!empty($this->relationPresets['filehook'])) {
            $relObj = ModUtil::apiFunc($this->name, 'selection', 'getEntity', array('ot' => 'file', 'id' => $this->relationPresets['filehook']));
            if ($relObj != null) {
                $relObj->addHookfile($entity);
            }
        }
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        // redirect to the list of hookobjects
    ...
    protected function getDefaultMessage($args, $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                        if ($this->mode == 'create') {
                            $message = $this->__('Done! Hookobject created.');
                        } else {
                            $message = $this->__('Done! Hookobject updated.');
                        }
                        break;
            case 'delete':
                        $message = $this->__('Done! Hookobject deleted.');
                        break;
            default:
                        $message = $this->__('Done! Hookobject updated.');
                        break;
