    ...
    {/if}
            ...
            <tr class="{cycle values='z-odd, z-even'}">
                ...
                <td headers="hAliascollection" class="z-left">
                    {if isset($file.Aliascollection) && $file.Aliascollection ne null}
                        ...
                        {/strip}</a>
                        <a id="collectionItem{$file.id}_rel_{$file.Aliascollection.id}Display" href="{modurl modname='MUFiles' type=$lct func='display' id=$file.Aliascollection.id ot='collection' theme='Printer' forcelongurl=true}" title="{gt text='Open quick view window'}" class="z-hide">{icon type='view' size='extrasmall' __alt='Quick view'}</a>
        ...
        {foreachelse}
            <tr class="z-{if $lqt eq 'admin'}admin{else}data{/if}tableempty">
              <td class="z-left" colspan="{if $lqt eq 'admin'}7{else}6{/if}">
