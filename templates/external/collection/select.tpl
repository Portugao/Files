...
{if $properties ne null && is_array($properties)}
    ...
    {foreach item='property' from=$properties}
        <p>
            {modapifunc modname='MUFiles' type='category' func='hasMultipleSelection' ot='collection' registry=$property assign='hasMultiSelection'}
            ...
            &nbsp;
            {selector_category name="`$baseID`_`$categorySelectorName``$property`" field='id' selectedValue=$catIds.$propertyName categoryRegistryModule='MUFiles' categoryRegistryTable=$objectType categoryRegistryProperty=$property defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize}
            <br{$break} />
        </p>
    {/foreach}
    {/nocache}
