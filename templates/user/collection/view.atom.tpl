...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUFiles' type='user' func='display' ot='collection' id=$items[0].id}{/capture}
    <id>{$uniqueID}</id>
...
{foreach item='collection' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='MUFiles' type='user' func='display' ot='collection' id=$collection.id fqurl='1'}" />

        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$collection.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUFiles' type='user' func='display' ot='collection' id=$collection.id}{/capture}
        <id>{$uniqueID}</id>
