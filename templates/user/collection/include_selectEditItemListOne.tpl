...
{if isset($item) && is_array($item) && isset($item[0]) && !is_object($item[0])}
    {modapifunc modname='MUFiles' type='selection' func='getEntity' objectType='collection' id=$item[0] assign='item'}
{/if}
...
<li id="{$idPrefixItem}">
    ...
    <a id="{$idPrefixItem}Edit" href="{modurl modname='MUFiles' type='user' func='edit' ot='collection' id=$item.id forcelongurl=true}">{$editImage}</a>
