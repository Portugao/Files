...
{assign var='objectTypes' value=$plugin_parameters.MUFiles_NewsletterPlugin_ItemList.param.ObjectTypes}
{assign var='pageArgs' value=$plugin_parameters.MUFiles_NewsletterPlugin_ItemList.param.Args}
...
{foreach key='objectType' item='objectTypeData' from=$objectTypes}
    ...
    <div class="z-formrow">
        ...
        <input id="plugin_{$i}_enable_{$j}" type="checkbox" name="MUFilesObjectTypes[{$objectType}]" value="1"{if $objectTypeData.nwactive} checked="checked"{/if} />
    </div>
    <div id="plugin_{$i}_suboption_{$j}">
        <div class="z-formrow">
            <label for="mUFilesArgs_{$objectType}_sorting">{gt text='Sorting'}:</label>
                <select id="mUFilesArgs_{$objectType}_sorting" name="MUFilesArgs[{$objectType}][sorting]">
