...
<form action="{$modvars.ZConfig.entrypoint|default:'index.php'}" method="get" id="mufilesCollectionQuickNavForm" class="mufilesQuickNavForm">
    <fieldset>
        ...
        {if !isset($categoryFilter) || $categoryFilter eq true}
            {modapifunc modname='MUFiles' type='category' func='getAllProperties' ot=$objectType assign='properties'}
            {if $properties ne null && is_array($properties)}
                ...
                {foreach key='propertyName' item='propertyId' from=$properties}
                    ...
                    &nbsp;
                    {selector_category name="`$categorySelectorName``$propertyName`" field='id' selectedValue=$catIdList.$propertyName categoryRegistryModule='MUFiles' categoryRegistryTable=$objectType categoryRegistryProperty=$propertyName defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize}
                {/foreach}
                {/nocache}
            {/if}
        {/if}
        {if !isset($collectionFilter) || $collectionFilter eq true}
            ...
            {modapifunc modname='MUFiles' type='selection' func='getEntities' ot='collection' slimMode=true assign='listEntries'}
            <select id="parent" name="parent">
                <option value="">{$lblDefault}</option>
            {foreach item='option' from=$listEntries}
                {assign var='entryId' value=$option.id}
                <option value="{$entryId}"{if $entryId eq $parent} selected="selected"{/if}>{$option.name}</option>
            {/foreach}
            </select>
        {/if}
...
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        mufilesInitQuickNavigation('collection', 'admin');
