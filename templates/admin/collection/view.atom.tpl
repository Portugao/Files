...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUFiles' type='admin' func='display' ot='collection' id=$items[0].id}{/capture}
    <id>{$uniqueID}</id>
    <updated>{$items[0].updatedDate|default:$smarty.now|dateformat:'%Y-%m-%dT%H:%M:%SZ'}</updated>
{/if}
    <link rel="alternate" type="text/html" hreflang="{lang}" href="{modurl modname='MUFiles' type='admin' func='main' fqurl=1}" />
...
{foreach item='collection' from=$items}
    <entry>
        <title type="html">{$collection->getTitleFromDisplayPattern()|notifyfilters:'mufiles.filterhook.collections'}</title>
        <link rel="alternate" type="text/html" href="{modurl modname='MUFiles' type='admin' func='display' ot='collection' id=$collection.id fqurl='1'}" />

        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$collection.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUFiles' type='admin' func='display' ot='collection' id=$collection.id}{/capture}
        <id>{$uniqueID}</id>
