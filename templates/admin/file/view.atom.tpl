{* purpose of this template: files atom feed in admin area *}
...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUFiles' type='admin' func='display' ot='file' id=$items[0].id}{/capture}
    <id>{$uniqueID}</id>
...
{foreach item='file' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='MUFiles' type='admin' func='display' ot='file' id=$file.id fqurl='1'}" />

        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$file.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUFiles' type='admin' func='display' ot='file' id=$file.id}{/capture}
        <id>{$uniqueID}</id>
