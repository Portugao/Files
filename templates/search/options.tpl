...
<div>
    <input type="checkbox" id="mUFilesCollections" name="mUFilesSearchTypes[]" value="collection"{if $active_collection} checked="checked"{/if} />
    <label for="active_mUFilesCollections">{gt text='Collections' domain='module_mufiles'}</label>
</div>
<div>
    <input type="checkbox" id="mUFilesFiles" name="mUFilesSearchTypes[]" value="file"{if $active_file} checked="checked"{/if} />
