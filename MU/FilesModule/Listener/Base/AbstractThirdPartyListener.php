...
abstract class AbstractThirdPartyListener implements EventSubscriberInterface
{
    ...
    {
        $collection = new Container('MUFilesModule');
        ...
        if (count($amounts) > 0) {
            foreach ($amounts as $amountInfo) {
                ...
                $route = 'mufilesmodule_' . strtolower($amountInfo['objectType']) . '_adminview';
                $routeArgs = [
     ...
     * MUFilesModule will use this to add a javascript helper to add custom items.
     *
     * You can access general data available in the event.
     *
     * The event name:
    ...
    public function getEditorHelpers(EditorHelperEvent $event)
    {
        ...
        $event->getHelperCollection()->add(
            [
                ...
                'type' => 'javascript',
                'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/mufiles/js/MUFilesModule.Finder.js'
            ]
        );
    }
    ...
    public function getCKEditorPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'mufilesmodule',
            'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/mufiles/scribite/CKEditor/mufilesmodule/',
            'file' => 'plugin.js',
            'img' => 'ed_mufilesmodule.gif'
        ]);
    }
    ...
    public function getQuillPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'mufilesmodule',
            'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/mufiles/scribite/Quill/mufilesmodule/plugin.js'
        ]);
    }
    ...
    public function getSummernotePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'mufilesmodule',
            'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/mufiles/scribite/Summernote/mufilesmodule/plugin.js'
        ]);
    }
    ...
    public function getTinyMcePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'mufilesmodule',
            'path' => $this->requestStack->getCurrentRequest()->getBasePath() . '/web/modules/mufiles/scribite/TinyMce/mufilesmodule/plugin.js'
        ]);
    }
}
