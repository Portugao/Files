...
function mUFilesInitInlineEditingWindow(objectType, idPrefix, containerId, inputType) {
    ...
    jQuery.each(mUFilesInlineEditHandlers, function (key, editHandler) {
        ...
        editHandler.windowInstanceId = mUFilesCreateInlineEditingWindowInstance(jQuery('#' + containerId));
    });
    ...
    newEditHandler = {
        ...
        inputType: inputType,
        windowInstanceId: mUFilesCreateInlineEditingWindowInstance(jQuery('#' + containerId))
    };

    // add it to the list of edit handlers
    mUFilesInlineEditHandlers.push(newEditHandler);
}
...
function mUFilesDetermineInputReference(objectType, alias, idPrefix, inputType, targetWindow) {
    ...
    inputPrefix = targetWindow.jQuery('.mufiles-edit-form').first().attr('name');
    inputField = null;
...
function mUFilesInitInlineEditingButtons(objectType, alias, idPrefix, inputType, createUrl) {
    ...
    if (jQuery('#' + createButtonId).length < 1) {
        ...
        createButton = jQuery('<a />', {
            ...
            class: 'btn btn-default mufilesmodule-inline-button'
        }).append(
    ...
    createButton.click(function (event) {
        event.preventDefault();
        mUFilesInitInlineEditingWindow(objectType, idPrefix, createButtonId, inputType);
    });
    ...
    jQuery.each(itemIdsArr, function (key, existingId) {
        ...
        if (existingId) {
            ...
            jQuery('#' + elemPrefix).click(function (event) {
                event.preventDefault();
                mUFilesInitInlineEditingWindow(objectType, idPrefix, elemPrefix, inputType);
            });
        }
    });
}
...
function mUFilesCloseWindowFromInside(idPrefix, itemId, formattedTitle, searchTerm) {
    ...
    jQuery.each(window.parent.mUFilesInlineEditHandlers, function (key, editHandler) {
        ...
        inputReference = mUFilesDetermineInputReference(editHandler.objectType, editHandler.alias, idPrefix, inputType, window.parent);
        if (null === inputReference || null === inputReference.field) {
            return;
        }
        ...
        window.parent.mUFilesSimpleAlert(anchorElement, window.parent.Translator.__('Information'), window.parent.Translator.__('Action has been completed.'), 'actionDoneAlert', 'success');
