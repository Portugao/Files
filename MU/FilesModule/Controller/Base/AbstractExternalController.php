...
abstract class AbstractExternalController extends AbstractController
{
    ...
    public function displayAction(Request $request, $objectType, $id, $source, $displayMode)
    {
        ...
        if (!$this->get('mu_files_module.permission_helper')->mayRead($entity)) {
            return '';
        }
    ...
    public function finderAction(Request $request, $objectType, $editor, $sort, $sortdir, $pos = 1, $num = 0)
    {
        ...
        $cssAssetBag->add($assetHelper->resolve('@MUFilesModule:css/style.css'));
        $cssAssetBag->add([$assetHelper->resolve('@MUFilesModule:css/custom.css') => 120]);
        
        $listEntriesHelper = $this->get('mu_files_module.listentries_helper');
        ...
        if (!in_array($objectType, $activatedObjectTypes)) {
            ...
            $redirectUrl = $this->get('router')->generate('mufilesmodule_external_finder', ['objectType' => array_shift($activatedObjectTypes), 'editor' => $editor]);
        
            return new RedirectResponse($redirectUrl);
        }
        
        if (!$this->get('mu_files_module.permission_helper')->hasComponentPermission($objectType, ACCESS_COMMENT)) {
            throw new AccessDeniedException();
        }
        ...
        $form = $this->createForm('MU\FilesModule\Form\Type\Finder\\' . ucfirst($objectType) . 'FinderType', $templateParameters, $formOptions);
        ...
        $output = $this->renderView('@MUFilesModule/External/' . ucfirst($objectType) . '/find.html.twig', $templateParameters);
