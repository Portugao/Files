...
abstract class AbstractNotificationHelper
{
    ...
    protected function collectRecipients($debug = false)
    {
        ...
        if ($this->recipientType == 'moderator' || $this->recipientType == 'superModerator') {
            ...
            $moderatorGroupId = $this->variableApi->get('MUFilesModule', 'moderationGroupFor' . $modVarSuffix, GroupsConstant::GROUP_ID_ADMIN);
            if ($this->recipientType == 'superModerator') {
                $moderatorGroupId = $this->variableApi->get('MUFilesModule', 'superModerationGroupFor' . $modVarSuffix, GroupsConstant::GROUP_ID_ADMIN);
            }
    ...
    protected function sendMails()
    {
        ...
        foreach ($this->recipients as $recipient) {
            ...
            $body = $this->templating->render('@MUFilesModule/' . $template, [
                'recipient' => $recipient,
                'mailData' => $mailData
            ]);
            $altBody = '';
    ...
    protected function prepareEmailData()
    {
        ...
        $hasDisplayAction = in_array($objectType, ['collection', 'file']);
        $hasEditAction = in_array($objectType, ['collection', 'file']);
        $routeArea = in_array($this->recipientType, ['moderator', 'superModerator']) ? 'admin' : '';
        $routePrefix = 'mufilesmodule_' . strtolower($objectType) . '_' . $routeArea;
