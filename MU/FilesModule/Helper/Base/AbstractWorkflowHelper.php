...
abstract class AbstractWorkflowHelper
{
    ...
    public function executeAction(EntityAccess $entity, $actionId = '', $recursive = false)
    {
        ...
        $logArgs = ['app' => 'MUFilesModule', 'user' => $this->currentUserApi->get('uname')];
    ...
    public function collectAmountOfModerationItems()
    {
        ...
        if ($this->permissionApi->hasPermission('MUFilesModule:' . ucfirst($objectType) . ':', '::', ACCESS_ADD)) {
            $amount = $this->getAmountOfModerationItems($objectType, $state);
            if ($amount > 0) {
                $amounts[] = [
                    'aggregateType' => 'collectionsApproval',
                    'description' => $this->translator->__('Collections pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->translator->_fn('One collection is waiting for approval.', '%amount% collections are waiting for approval.', $amount, ['%amount%' => $amount])
                ];
        
                $this->logger->info('{app}: There are {amount} {entities} waiting for approval.', ['app' => 'MUFilesModule', 'amount' => $amount, 'entities' => 'collections']);
            }
        }
        $objectType = 'file';
        if ($this->permissionApi->hasPermission('MUFilesModule:' . ucfirst($objectType) . ':', '::', ACCESS_ADD)) {
            ...
            if ($amount > 0) {
                $amounts[] = [
                    ...
                    'description' => $this->translator->__('Files pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->translator->_fn('One file is waiting for approval.', '%amount% files are waiting for approval.', $amount, ['%amount%' => $amount])
                ];
        
                $this->logger->info('{app}: There are {amount} {entities} waiting for approval.', ['app' => 'MUFilesModule', 'amount' => $amount, 'entities' => 'files']);
            }
        }
