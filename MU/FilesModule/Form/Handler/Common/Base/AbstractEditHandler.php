...
namespace MU\FilesModule\Form\Handler\Common\Base;
...
use MU\FilesModule\Helper\HookHelper;
...
abstract class AbstractEditHandler
{
    ...
    public function processForm(array $templateParameters = [])
    {
        ...
        // default to referer
        $refererSessionVar = 'mufilesmodule' . $this->objectTypeCapital . 'Referer';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'MUFilesModule' . $this->objectTypeCapital . $entity->getKey();
            ...
            $permissionLevel = in_array($this->objectType, ['collection', 'file']) ? ACCESS_COMMENT : ACCESS_EDIT;
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'MUFilesModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->getKey()];
    ...
    public function handleCommand(array $args = [])
    {
        ...
        if ($entity->supportsHookSubscribers()) {
            ...
            if ($hasDisplayAction && $action != 'delete') {
                ...
                $routeUrl = new RouteUrl('mufilesmodule_' . $this->objectTypeLower . '_display', $urlArgs);
            }
    ...
    protected function addDefaultMessage(array $args = [], $success = false)
    {
        ...
        $logArgs = ['app' => 'MUFilesModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->getKey()];
        if (true === $success) {
    ...
    public function fetchInputData()
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->requestStack->getCurrentRequest()->getSession()->set('MUFilesModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
    ...
    protected function prepareWorkflowAdditions($enterprise = false)
    {
        ...
        $groupApplicationArgs = [
            'user' => $currentUserId,
            'group' => $this->variableApi->get('MUFilesModule', 'moderationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
        ];
        ...
        if (true === $enterprise) {
            $groupApplicationArgs = [
                'user' => $currentUserId,
                'group' => $this->variableApi->get('MUFilesModule', 'superModerationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
            ];
